...
export async function performPrismaBackup(): Promise<void> {
  ...
  try {
    ...
    // Prefer an existing SQL schema dump if available (generated externally)
    const possibleOnlineSchema = path.join(BACKUP_DIR, 'prisma_online_backup.sql');
    const primarySchemaFile = fs.existsSync(possibleOnlineSchema) ? possibleOnlineSchema : schemaBackupPath;

    // If we have both schema SQL (DDL) and data INSERTs, create a combined restore file
    let backupToSend: string;
    if (dataBackupPath) {
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const combinedFileName = `sajag_restore_${timestamp}.sql`;
      const combinedFilePath = path.join(BACKUP_DIR, combinedFileName);

      try {
        const schemaSql = fs.existsSync(primarySchemaFile) ? fs.readFileSync(primarySchemaFile, 'utf8') : '';
        const dataSql = fs.readFileSync(dataBackupPath, 'utf8');

        // Ensure schema (DDL) comes before data INSERTs
        const combinedContent = [schemaSql, '\n\n-- DATA INSERTS (generated by prisma backup)\n', dataSql].join('');

        fs.writeFileSync(combinedFilePath, combinedContent, 'utf8');
        console.log(`✅ Created combined restore file: ${combinedFileName}`);
        backupToSend = combinedFilePath;
      } catch (err: any) {
        console.warn('⚠️ Failed to create combined restore file, falling back to data or schema file:', err?.message || err);
        backupToSend = dataBackupPath || schemaBackupPath;
      }
    } else {
      // No data backup, send schema-only backup
      backupToSend = primarySchemaFile;
    }
